...
## DESCRIPTION
## -----------------------------------------------------------------------------
## Builds the lambda (task) zip file for perform_orca_reconcile.
##
##
## USAGE
## -----------------------------------------------------------------------------
...
cp ../package/awslambda-psycopg2/psycopg2-3.9/* build14/psycopg2/
...
cp -r schemas/ build14/
check_returncode $? "ERROR: Failed to copy schema files to build directory."

## Create the zip archive
cd build14
trap 'cd -;deactivate_and_delete_venv;rm -rf build14;' EXIT
run_and_check_returncode "zip -qr ../perform_orca_reconcile.zip ."
