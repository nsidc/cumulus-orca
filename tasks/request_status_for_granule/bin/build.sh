...
## DESCRIPTION
## -----------------------------------------------------------------------------
## Builds the lambda (task) zip file for request_status_for_granule.
##
##
## USAGE
## -----------------------------------------------------------------------------
...
cp ../package/awslambda-psycopg2/psycopg2-3.9/* build20/psycopg2/
...
cp -r schemas/ build20/
check_returncode $? "ERROR: Failed to copy schema files to build directory."

## Create the zip archive
cd build20
trap 'cd -;deactivate_and_delete_venv;rm -rf build20;' EXIT
run_and_check_returncode "zip -qr ../request_status_for_granule.zip ."
