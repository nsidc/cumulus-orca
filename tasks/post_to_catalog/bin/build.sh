...
## DESCRIPTION
## -----------------------------------------------------------------------------
## Builds the lambda (task) zip file for post_to_catalog.
##
##
## USAGE
## -----------------------------------------------------------------------------
...
cp ../package/awslambda-psycopg2/psycopg2-3.9/* build16/psycopg2/
...
cp -r schemas/ build16/
check_returncode $? "ERROR: Failed to copy schema files to build directory."

## Create the zip archive
cd build16
trap 'cd -;deactivate_and_delete_venv;rm -rf build16;' EXIT
run_and_check_returncode "zip -qr ../post_to_catalog.zip ."
