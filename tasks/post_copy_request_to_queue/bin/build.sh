...
## DESCRIPTION
## -----------------------------------------------------------------------------
## Builds the lambda (task) zip file for post_copy_request_to_queue.
##
##
## USAGE
## -----------------------------------------------------------------------------
...
cp ../package/awslambda-psycopg2/psycopg2-3.9/* build15/psycopg2/
...
cp -r schemas/ build15/
check_returncode $? "ERROR: Failed to copy schema files to build directory."

## Create the zip archive
cd build15
trap 'cd -;deactivate_and_delete_venv;rm -rf build15;' EXIT
run_and_check_returncode "zip -qr ../post_copy_request_to_queue.zip ."
